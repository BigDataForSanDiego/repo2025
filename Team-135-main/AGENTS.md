...
### Cross-Platform Parity (HomeBase)
Whenever you modify a HomeBase screen/component for web (`*.web.tsx`), apply the same functional and layout changes to the corresponding native file (`*.native.tsx`). Keep Expo/RN behavior in sync across iOS, Android, and web before considering a task complete.
