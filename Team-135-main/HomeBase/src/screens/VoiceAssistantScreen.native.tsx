...
let InlineMapView: any;
let InlineMarker: any;
if (Platform.OS !== 'web') {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const maps = require('react-native-maps');
  InlineMapView = maps.default;
  InlineMarker = maps.Marker;
}
...
export const VoiceAssistantScreen: React.FC = () => {
  ...
  const addressPanelHeight = Math.max(Math.min(screenHeight * 0.42, 360), 300);
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <LinearGradient
        colors={effectiveMode === 'emergency'
          ? ['#1a0505', '#2d0a0a', '#1a0505']
          : ['#050408', '#100f1f', '#050408']
        }
        ...
        <View style={styles.contentWrapper}>
          ...
          >
            <ScrollView
              ...
              scrollEnabled={showAddressPanel}
              bounces={showAddressPanel}
            >
              <Animated.View
                style={[
              ...
              {showAddressPanel && effectiveSelectedResource ? (
                <>
                  ...
                  <View style={styles.resourceMicSection}>
                    <View style={styles.resourceMicCompactWrapper}>
                      <VoiceButton
                        mode={buttonMode}
                  ...
                  >
                    <Animated.View
                      style={[
                        styles.waveLayer,
                        {
                          backgroundColor: effectiveMode === 'emergency'
                            ? 'rgba(220,38,38,0.15)'
                            : 'rgba(99,102,241,0.12)',
                          borderColor: effectiveMode === 'emergency'
                            ? 'rgba(220,38,38,0.08)'
                    ...
                    <LinearGradient
                      colors={effectiveMode === 'emergency'
                        ? ['rgba(220,38,38,0.65)', 'rgba(185,28,28,0.45)']
                        : ['rgba(99,102,241,0.65)', 'rgba(14,165,233,0.35)']
                      }
                      start={{ x: 0, y: 0 }}
                      end={{ x: 1, y: 1 }}
...
const styles = StyleSheet.create({
  ...
  liveTranscriptDefaultSpacing: {
    // Deprecated - spacing now unified in liveTranscriptContainer
  },
...
const InlineResourcePanel = ({ resource, userLocation, onClose, height }: InlineResourcePanelProps) => {
  const hasMap = InlineMapView && userLocation;

  const region = hasMap
    ? {
        latitude: (userLocation.latitude + resource.latitude) / 2,
        longitude: (userLocation.longitude + resource.longitude) / 2,
        latitudeDelta: Math.max(Math.abs(userLocation.latitude - resource.latitude) * 2.5, 0.01),
        longitudeDelta: Math.max(Math.abs(userLocation.longitude - resource.longitude) * 2.5, 0.01),
      }
    : null;
  ...
  return (
    <View style={[styles.inlinePanel, styles.inlinePanelOffset, { height }]}>
      <View style={[styles.inlineMapContainer, { height }]}>
        {hasMap && region ? (
          <InlineMapView
            style={{ flex: 1 }}
          ...
          <LinearGradient
            ...
            style={[styles.inlineMapPlaceholder, { borderRadius: 0 }]}
          >
...
const DemoSceneControls = ({ scenario, onSelect }: DemoSceneControlsProps) => {
  ...
  return (
    <View style={[styles.demoControlsWrapper, collapsed && styles.demoCollapsedWrapper]}>
      ...
      <TouchableOpacity
        style={[
          styles.demoToggleHandle,
          collapsed && styles.demoToggleHandleCollapsed,
        ]}
