...
function normalizeRow(r){
  ...
  const type = (r.type || r.Type || r.category || r.Category || '').toLowerCase();
  ...
  const contact = r.phone || r.Phone || r.contact || r.Contact || '';
  const status = r.status || r.Status || '';
  const wait_minutes = toFloat(r.wait || r.wait_minutes || r.Wait);
  const hours_json = r.hours || r.Hours || '';
  const last_verified_at = r.verified_at || r.last_verified_at || r.Verified || '';
